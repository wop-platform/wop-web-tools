...
(function (root) {
  ...
  var DICT = {
    zh: {
      ...
      'main.bld.sm2only': '本页请求构造仅支持 RSA 三套件；WOP-SM2-SM3 请切换到「国密」标签页（国密请求构造区，SM2 签名 + SM4-GCM 加密）',
      ...
      'main.kg.bits': '密钥长度',
      'main.kg.bits.2048': '2048 位（默认）',
      'main.kg.bits.2048.s': '对应安全套件 WOP-RSA2048-SHA256',
      'main.kg.bits.3072': '3072 位',
      'main.kg.bits.3072.s': '对应安全套件 WOP-RSA3072-SHA256',
      'main.kg.bits.4096': '4096 位',
      'main.kg.bits.4096.s': '对应安全套件 WOP-RSA4096-SHA256',
      'main.kg.bits.note': '注：密钥长度支持 2048 / 3072 / 4096 位，默认 2048 位。如被要求生成「3074 位密钥」，即为笔误——RSA 不存在 3074 位，请选择 3072 位。本页请求构造支持 WOP-RSA2048-SHA256 / WOP-RSA3072-SHA256 / WOP-RSA4096-SHA256 套件；国密 WOP-SM2-SM3 请切换到「国密」标签页构造与验证。',
      ...
      'main.kg.privfmt.b64': '单行 Base64（默认 · PKCS#8 DER · 网关程序对接格式）',
      'main.kg.privfmt.pem': 'PKCS#8 PEM（平台文档格式）',
      'main.kg.pub.h': '商户公钥',
      'main.kg.pub.hint': '（上传至开放平台）',
      'main.kg.pubfmt': '公钥格式',
      'main.kg.pubfmt.b64': '单行 Base64（默认 · SPKI DER · 网关程序对接格式）',
      'main.kg.pubfmt.pem': 'X.509 SPKI PEM（平台文档格式）',
      'main.kg.show': '显示',
      ...
      'main.req.out.h': '构造结果',
      'main.req.path': '请求路径',
      'main.req.plat.help': '联调模式：本地生成模拟平台密钥对（位数跟随商户密钥；私钥仅存本次会话内存，用于「模拟平台响应」），生产环境请粘贴平台下发的真实公钥。',
      ...
      'main.sim.platform': '已生成联调平台密钥对（RSA-{bits}，仅本会话内存）',
      ...
      'main.st.h': '黄金向量自测',
      'main.st.note': '内置 wop-specs 黄金向量（fixture 只读副本，禁手改），一键跑字节级断言：正向量（摘要 / AES-GCM 密文 / DEK 载荷 / OAEP 解包）必须一致，负向量（跨族 / 格式错误 / 尾位非规范 / MGF1 陷阱）必须拒绝；rsa2048 商户签名因上游黄金向量暂无 2048 条目，以 TEST-ONLY 教学向量对拍。全部通过才显示绿色横幅（横幅含教学向量注记）。',
      'main.st.pass': '✓ {n} 项断言全部通过 —— 黄金向量对齐（v1.0-ratified F8）；rsa2048 商户签名为 TEST-ONLY 教学向量对拍',
      'main.st.res.h': '断言结果',
      'main.st.run': '运行自测',
      'main.st.running': '运行中…',
      'main.tab.api': 'API 目录',
      ...
      'wf12.sub': '签名与信封加密的层级关系：L2 报文的 digest 摘要对象是密文、DEK 经 RSA-OAEP 包装；演示密钥均为 TEST-ONLY（digest/信封对拍 wop-specs 黄金向量，商户签名钥 rsa2048 为本地教学向量）',
      ...
      'wf-gm.req.appkey': 'x-wop-appkey（必填，SM2 userId）',
    ...
    en: {
      ...
      'wf12.sub': 'Layering of signing and envelope encryption: the digest of an L2 message covers ciphertext, and the DEK is wrapped with RSA-OAEP; demo keys are TEST-ONLY (digest/envelope anchor to wop-specs golden vectors; the rsa2048 merchant signing key is a local teaching vector)',
      ...
      'wf-gm.req.appkey': 'x-wop-appkey (required, SM2 userId)',
      ...
      'main.kg.bits.2048': '2048-bit (default)',
      'main.kg.bits.2048.s': 'Suite WOP-RSA2048-SHA256',
      'main.kg.bits.3072': '3072-bit',
      'main.kg.bits.3072.s': 'Suite WOP-RSA3072-SHA256',
      'main.kg.bits.4096': '4096-bit',
      'main.kg.bits.4096.s': 'Suite WOP-RSA4096-SHA256',
      'main.kg.bits.note': 'Note: key lengths 2048 / 3072 / 4096 bits are supported, defaulting to 2048. If asked for a "3074-bit key", that is a typo — RSA has no 3074-bit size; choose 3072. Request construction on this page supports the WOP-RSA2048-SHA256 / WOP-RSA3072-SHA256 / WOP-RSA4096-SHA256 suites; for WOP-SM2-SM3 switch to the SM tab.',
      ...
      'main.kg.privfmt.b64': 'Single-line Base64 (default · PKCS#8 DER · gateway integration format)',
      'main.kg.privfmt.pem': 'PKCS#8 PEM (platform doc format)',
      'main.kg.pub.h': 'Merchant public key',
      'main.kg.pub.hint': '(upload to the open platform)',
      'main.kg.pubfmt': 'Public key format',
      'main.kg.pubfmt.b64': 'Single-line Base64 (default · SPKI DER · gateway integration format)',
      'main.kg.pubfmt.pem': 'X.509 SPKI PEM (platform doc format)',
      'main.kg.show': 'Show',
      ...
      'main.req.plat.help': 'Debug mode: generate a simulated platform keypair locally (modulus follows the merchant key; private key kept in session memory only, used by "Simulate platform response"); in production paste the real platform public key.',
      ...
      'main.sim.platform': 'Debug platform keypair generated (RSA-{bits}, session memory only)',
      ...
      'main.st.note': 'Built-in wop-specs golden vectors (read-only fixture copy, do not edit) run byte-level assertions in one click: positives (digest / AES-GCM ciphertext / DEK blob / OAEP unwrap) must match, negatives (cross-family / malformed / non-canonical trailing bits / MGF1 trap) must be rejected; the rsa2048 merchant signature is checked against a TEST-ONLY teaching vector (no 2048 entry in upstream golden vectors yet). Only a full pass shows the green banner (which carries the teaching-vector note).',
      'main.st.pass': '✓ All {n} assertions passed — aligned with the wop-specs golden vectors (v1.0-ratified F8); the rsa2048 merchant signature is checked against a TEST-ONLY teaching vector',

 ...
 * 双环境兼容：浏览器以 <script> 加载挂 window.GM_PAGE_SELFTEST；node 以 new Function('window', code) 捕获。
...
function GM_PAGE_SELFTEST() {
  ...
  guard('GM-P5 国密请求构造', function () {
    ...
    if (lines.length !== 8) throw new Error('canonical 行数非 4 段+4 头: ' + lines.length);
    if (lines[4].indexOf('x-wop-content-digest:') !== 0) throw new Error('canonicalHeaders 首头非 digest');

...
selftest 共 14 条断言，覆盖任务书全部 5 类要求 + 否定式条款：
...
| WF11-VALID-REQ | 校验：嵌套子错误不被对象必填掩盖 | 嵌套对象内类型错误被「对象必填缺失」整体报错吞掉（advisory 复审发现，子错误改为无条件上浮） |
...
- 复审（advisory）又抓到 1 个相邻 bug 并修复：`collectValues` 在嵌套对象「无任何成功子值」时
  吞掉子错误、误报「对象必填缺失」；修复后子错误无条件上浮（吞错场景报
  `callback.url | callback.retryTimes` 精确路径），由「嵌套子错误不被对象必填掩盖」断言持续守卫。
